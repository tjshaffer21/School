...
{
    double a = (-1*(z_max+z_min))/(z_max-z_min);
    double b = (2*(z_max*z_min))/(z_max-z_min);
...
Matrix* AffineTransforms::buildCamera( Vertex* eye, Vertex* at, Vector* up )
{
    ...
    double v_scale = (-1*(up->dotProduct(n)))/sqrt(1-pow(up->dotProduct(n),2));
    double v_scale1 = 1/sqrt(1-pow(up->dotProduct(n),2));
    double x = v_scale*n->getX() + v_scale1*up->getX();
    double y = v_scale*n->getY() + v_scale1*up->getY();
    double z = v_scale*n->getZ() + v_scale1*up->getZ();
