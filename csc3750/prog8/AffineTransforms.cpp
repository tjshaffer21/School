...
*    Preconditions:
*        A double - The width of the window.
*        A double - The height of the window.
*    Postconditions:
*        Returns a pointer to the W matrix.
*/
...
*        A double - The height of the window.
*    Postconditions:
*        Returns a pointer to the A matrix.
*/
Matrix* AffineTransforms::aspectRatio( int width, int height, double fov )
{
...
*    Preconditions:
*        A double for the x translation.
*        A double for the y translation.
*        A double for the z translation.
*    Postconditions:
...
Matrix* AffineTransforms::perspectiveNorm( double fovx, double ratio,
    double z_max, double z_min )
{
    double angle = RADIANS*fovx;
    double xmax = tan(angle/2);     // Width of camera
    double ymax = xmax*ratio;       // Height of camera.
...
/**
*    buildCamera
*    Creates a matrix that creates a camera for the scene.
*    Preconditions:
*        A vertex pointer
*        A vertex pointer
*        A vertex pointer
*    Postconditions:
*        Returns a pointer to the C matrix.
*/
Matrix* AffineTransforms::buildCamera( Vertex* eye, Vertex* at, Vector* up )
{
    ...
    double num1 = -1*(eye->getX()*u->getX() + eye->getY()*u->getY() +
        eye->getZ()*u->getZ() );
    double num2 = -1*(eye->getX()*v->getX() + eye->getY()*v->getY() +
        eye->getZ()*v->getZ() );
    double num3 = -1*(eye->getX()*n->getX() + eye->getY()*n->getY() +
        eye->getZ()*n->getZ() );
